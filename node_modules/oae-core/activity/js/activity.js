...
define(['jquery', 'underscore', 'oae.core'], function($, _, oae) {
    ...
    return function(uid, showSettings, widgetData) {
        ...
        var setupActivityNotifications = function() {

            var _handleWebsocketNotification = function(ev, activity) {
                unReadActivities.push(activity);
                oae.api.util.template().render($('#activity-new-item-template', $rootel), {
                    'unReadActivities': unReadActivities.length
                }, $('#activity-new-items', $rootel));


                $('#activity-show-unread-activities').click(function() {
                    var data = {
                        'items': unReadActivities
                    };
                    infinityScroll.prependItems(data);
                    unReadActivities = [];

                    $('#activity-new-items').html('');
                    getActivity();
                });
            };

            // Express we're interested in new notifications
            oae.api.websockets.subscribe(widgetData.principalId, 'activity', widgetData.signature);

            // Listen for new notifications coming in via the Push API.
            $(document).on('push.' + widgetData.principalId + '#activity', _handleWebsocketNotification);
        };
