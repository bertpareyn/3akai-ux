    ...
    return function(uid, showSettings) {
                ...
                if (!value) {
                    ...
                    $.each(selectedLinks, function(index, file) {
                        if (file.displayName === prevValue) {
                            $listItemContainer.attr('data-id', value);
                            $listItemContainer.find('.btn.createlink-trash').attr('data-id', value);
        ...
        var initDeleteFromSelected = function() {
            $rootel.on('click', '.createlink-trash', function(ev) {
                var toTrash = $(this).parent().attr('data-id');
                $.each(selectedLinks, function(index, link) {
                    if (link._id === toTrash) {
        ...
        var initLinkCreation = function() {
            $('#createlink-doupload').off('click').on('click', function() {
                ...
                var doUpload = function(link) {
                    if (link) {
                        $('div[data-id="' + link._id + '"] .icon-spinner', $rootel).show();
                        oae.api.content.createLink(link.displayName, link.description, link.visibility, link.link, [], link.viewers, function(error, data) {
                            $('div[data-id="' + link._id + '"] .icon-spinner', $rootel).hide();
                            if (!error) {
                                $('div[data-id="' + link._id + '"] .icon-ok', $rootel).show();
                            } else {
                                $('div[data-id="' + link._id + '"] .icon-warning-sign', $rootel).show();
                            }
