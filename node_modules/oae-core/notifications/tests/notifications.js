...
casper.start('http://test.oae.com', function(){
	...
	casper.thenOpen('http://test.oae.com/me', function() {
		casper.waitForSelector('.activity-widget', function() {
			casper.wait(2000, function(){casper.capture('activity.png');});
			// Verify that the notifications can be opened
			casper.then(function() {
			...
			// Open the notifications
			casper.then(function() {
				casper.wait(2000, function(){openNotifications();
				casper.capture('notifications.png');});
			});

			// Verify the added to group notification
			casper.then(function() {
				casper.echo('Verify added to group notification');
				verifyAddToGroup(user1, group1, 5);
			});
			...
			casper.then(function() {
				...
				verifyCreateDiscussion(user1, discussionName1, 4);
			});

			// Verify the created file notification
			casper.then(function() {
				casper.echo('Verify created file notification');
				verifyCreatedFile(user1, 'balloons.jpg', fileUrl1, 3);
			});
			...
			casper.then(function() {
				...
				verifyCommentOnFile(user1, 'balloons.jpg', fileUrl1, 2);
			});

			// Verify the posted to discussion notification
			casper.then(function() {
				casper.echo('Verify post to discussion notification');
				verifyPostToDiscussion(user1, discussionName1, 1);
			});
		});
	});
