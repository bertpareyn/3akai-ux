 ...
 */
var verifyAddToGroup = function(user, group) {
 ...
 */
var verifyCreateDiscussion = function(user, discussionName) {
 ...
 */
var verifyCommentOnFile = function(user, fileName, fileUrl){
	...
	casper.test.assertTextExists('commented on the file',
		'The sentence "commented on the file" from the notification of created a discussion is present');
	casper.test.assertSelectorHasText('#notifications-container li a', fileName,
		'"filename" in the notification "\'username\' commented on the file \'filename\'" is present');
	casper.test.assertExists('#notifications-container li a[href="' + user.profilePath + '"]',
		'The link to the user from the notification of created a discussion is present');
	casper.test.assertExists('#notifications-container li a[href="' + fileUrl + '"]',
		'The link to the file from the notification of created a discussion is present');
};
 ...
 */
var verifyPostToDiscussion = function(user, discussionName){
	...
	casper.test.assertTextExists('posted to the discussion',
		'The sentence "posted to the discussion" from the notification of created a discussion is present');
	casper.test.assertSelectorHasText('#notifications-container li a', discussionName,
...
casper.start('http://test.oae.com', function(){
	...
	casper.thenOpen('http://test.oae.com/me', function() {
		casper.waitForSelector('.activity-widget', function() {
			casper.wait(1000, function(){casper.capture('groupactivity.png');});
			// Verify that the notifications can be opened
			casper.then(function() {
			...
			casper.then(function() {
				openNotifications();
				casper.wait(10000, function() {casper.capture('notifications.png');});
			})

			// Verify the added to group notification
			casper.then(function() {
				casper.echo('Verify added to group notification');
				verifyAddToGroup(user1, group1);
			});
			...
			casper.then(function() {
				...
				verifyCreateDiscussion(user1, discussionName1);
			});

			casper.then(function() {
				casper.echo('Verify comment on file notification');
				// balloons.jpg is the name of the file that gets uploaded in contentUtil()
				verifyCommentOnFile(user1, 'balloons.jpg', fileUrl1);
			});

			casper.then(function() {
				casper.echo('Verify post to discussion notification');
				verifyPostToDiscussion(user1, discussionName1);
			});
		});
	});
